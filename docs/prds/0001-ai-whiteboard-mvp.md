# Factory-Ready PRD: AI Whiteboard (personal tool first, SaaS-ready)

**Source:** user request (chat, 2026-09-16) + TubeGuruji transcript "Build Full Stack AI Agentic Whiteboard App using NextJs, React, Tailwindcss, Typescript, CodeRabbit" + `PLAN.md`
**Revision:** 2 (2026-09-16). Revision 1 was approved by the owner; revision 2 only records the owner's decisions D-002 (Railway) and D-003 (no billing, personal-tool phase) and the assumptions they imply. Any further material change needs a new approval note.
**Readiness:** READY WITH ASSUMPTIONS (all slices S-1 to S-8)
**Approval:** approved by owner (TayoAki) via chat on 2026-09-16 for revision 1 plus decisions D-002/D-003; auto-proceed not authorized. D-001 (create and protect `main`) is still an open owner action and blocks opening any slice PR.

## 1. Executive summary

Independent users need a whiteboard where an AI collaborator can draw and edit diagrams, wireframes and notes for them instead of only answering in text. Build a Next.js app with an Excalidraw canvas and a CopilotKit agent that reads the board, edits it through validated tools, asks before destructive changes, and is metered by server-enforced budgets. The tutorial proves the feature set; this PRD makes it testable and adds the enforcement the tutorial lacks. **Phase one is a personal tool for the owner deployed on Railway**: authentication and per-user scoping stay (they are the tenant boundary and keep the codebase SaaS-ready), but billing, plans, and teams are deferred. FluidVoice (macOS dictation) works as an input device from day one; deeper voice integration is a late slice.

## 2. Source facts and evidence

- **Supplied facts (user):** use CopilotKit; build an AI whiteboarding SaaS; inspired by the transcript; FluidVoice integration is "future maybe"; **PRD revision 1 approved; host on Railway; skip Stripe; use it first as a personal tool.**
...
- **External facts (verified 2026-09-16, see `PLAN.md` §2):** CopilotKit 1.72.0 with v2 entry points; Excalidraw 0.18.1 (MIT); tldraw license forbids production use without a commercial agreement; FluidVoice 1.6.0, macOS-only, GPLv3, no public API; Railway tooling is attached to this session.
- **Repository evidence:** planning documents only; no build, lint, or test commands exist yet; GitHub default branch is currently `claude/festive-fermi-r689ry` (first pushed branch).
- **Unknowns:** data-retention policy on account deletion (D-005); whether the product later opens to other users (out of scope for this revision).

## 3. Goals and success measures

| ID | Goal or measure | Target | How measured |
|---|---|---|---|
| G-001 | The owner can create a board and get an AI-generated diagram on the canvas | ≤ 3 minutes from sign-in, no manual JSON | Playwright e2e timing against the Railway deployment |
...
| G-003 | No cross-user data access, even though only the owner is admitted | 0 failures in the negative-path suite; every project-scoped route covered | integration tests + entry-point inventory test in CI |
| G-004 | AI cost is bounded | configured daily run cap cannot be exceeded; provider spend alert in place | runtime budget tests + provider dashboard |
...
| G-006 | The app runs unattended on Railway | health check green; a redeploy loses no boards | Railway deployment logs + e2e after redeploy |

## 4. Non-goals (this revision)

- Billing, plans, payment providers (Stripe or Clerk Billing), pricing pages. Deferred until the product leaves the personal-tool phase (D-003).
- Teams, organizations, shared boards, share links, realtime collaboration.
- Public sign-up: only allowlisted accounts are admitted (A-009).
...
|---|---|---|---|
| Visitor | see the landing page, attempt sign-in | `/`, `/sign-up`, `/sign-in` | no data access; non-allowlisted accounts are refused after authentication (FR-005) |
| Owner (allowlisted user) | create, edit, archive own boards; use the AI copilot | `/dashboard*`, `/workspace/[projectId]`, `/api/*` | may only act on boards where `projects.user_id` = their user; budgets apply |
...
| Railway (platform) | health check | `GET /api/health` | public, returns status only, no data |

Tenant model (A-001): the tenant is the user. Every board query is scoped by `user_id`; the schema keeps room for an `org_id` later. The allowlist (A-009) is a phase-one admission control, not a replacement for per-user scoping.

## 6. User and system flows

### Primary flow

1. Owner signs in (Clerk) → server checks the allowlist → upserts `users` row with 3 credits on first authenticated visit.
...
- Not signed in → `/dashboard` and `/workspace/*` redirect to sign-in; `/api/*` returns 401 (webhooks and health excepted).
- Signed in but not allowlisted → "This instance is private" page; 403 on `/api/*`; no `users` row created; `auth.denied` audit event.
...
- Railway redeploy → boards and scenes persist (Neon); in-memory chat threads reset (A-010); the UI starts a fresh thread without error.

## 7. Functional requirements

| ID | Requirement | Priority | Source | Notes |
|---|---|---|---|---|
| FR-001 | The system shall let a user sign in with Google, GitHub, or email + password via Clerk. | Must | transcript | prebuilt Clerk pages |
| FR-002 | The system shall create the user's `users` row server-side on the first authenticated, allowlisted request, with `credits = 3`, exactly once. | Must | transcript (client-side variant) | server-side upsert replaces the tutorial's client `POST` |
| FR-003 | The system shall deny unauthenticated access to `/dashboard*`, `/workspace/*`, and `/api/*` except signature-verified webhooks and `GET /api/health`. | Must | transcript + guardrails | middleware + per-route check |
...
| FR-005 | While in the personal-tool phase, the system shall admit only accounts whose email is on the server-side allowlist (`ALLOWED_EMAILS`), refusing others with a private-instance message and no data access; Clerk's own sign-up restriction is configured as a first layer. | Must | A-009 (derived from D-003) | remove the allowlist to open the product |
| FR-010 | The system shall create a board with a name of 1–60 characters when the user has ≥ 1 credit, decrementing credits atomically in the same transaction. | Must | transcript | single `UPDATE … WHERE credits > 0 RETURNING` inside the transaction |
...
| FR-016 | The system shall show credits used vs. the configured maximum in the sidebar. | Must | transcript | |
...
| FR-024 | Each save shall regenerate a ≤ 400 px preview and store it with the board. | Must | transcript | base64 (A-005) |
...
| FR-040 | AI runs shall be metered per user (configurable daily cap) and rate limited, enforced in the runtime, never only in the browser. | Must | transcript (credits) + guardrails | cost control for the owner; numbers in A-006 |
...
| FR-060 | Plan upgrades and payments. | **Deferred** | D-003 | out of scope for this revision; keep the id for a future revision |
| FR-070 | The app shall run as one long-lived Node service on Railway with the CopilotKit runtime in-process, a `GET /api/health` endpoint, configuration via Railway variables, and Neon as the database. | Must | D-002 | `InMemoryAgentRunner` accepted (A-010) |
...
- **Permission denied:** unauthenticated → sign-in redirect; not allowlisted → private-instance page; foreign board → "Board not found".
...
- **API/events/contracts:** JSON route handlers under `/api/*` with zod-validated bodies; errors `{ error: { code, message } }` with 400/401/402/403/404/413/429/500; the CopilotKit runtime speaks AG-UI over `/api/copilotkit/*`; audit events per `docs/architecture/GUARDRAIL_MAP.md`.
...
- **Deployment contract (FR-070):** Railway service built from the repository (`npm ci && npm run build`, start `npm run start`), health check `GET /api/health`, secrets only in Railway variables, Neon connection string via `DATABASE_URL`.

## 10. Quality requirements

| ID | Category | Requirement or threshold | Verification |
|---|---|---|---|
| NFR-001 | security (authorization) | Every project-scoped route resolves the principal from Clerk server-side, checks the allowlist, and scopes the query by `user_id`; foreign ids return 404 with no side effect. | negative integration tests + entry-point inventory test |
...
| NFR-003 | resource/cost limits | Scene ≤ 5 MB and ≤ 3 000 elements; files ≤ 2 MB; AI: `maxSteps` ≤ 8, bounded output tokens, per-user runs/minute and runs/day, provider monthly spend alert. | runtime tests with oversized fixtures; budget config test |
...
| NFR-005 | secrets | Provider and DB secrets exist only in server modules and Railway variables; client bundle contains none. | build-time grep in CI |
| NFR-006 | performance | Board load p75 ≤ 2 s for ≤ 500 elements; autosave round trip p75 ≤ 1 s; diagram generation p50 ≤ 15 s for ≤ 20 nodes (measured, then tuned). | e2e timings against Railway |
| NFR-007 | reliability | Autosave never loses local edits on transient failure; Neon point-in-time recovery enabled; a Railway redeploy loses no persisted data. | e2e with network fault injection; redeploy check |
...
| NFR-009 | observability | Error tracking with correlation ids on API and runtime; AI usage logged per run; Railway logs retained. | manual check on Railway |
...
- **Dependencies:** Clerk account (with sign-up restriction enabled); Neon project; Anthropic API key (A-002); Railway project; CodeRabbit connected to the repo; GitHub branch protection on `main` (D-001).
- **Fixed constraints:** CopilotKit as the copilot framework (user); Next.js App Router; Excalidraw (license); Railway hosting (D-002); no payment provider in this revision (D-003); no FluidVoice code in this repo (GPLv3).
- **Implementation preferences (nonbinding):** Tailwind + shadcn; Drizzle; Playwright + vitest; dagre for layout.

## 12. Acceptance criteria and evidence

| ID | Requirement links | Observable acceptance criterion | Planned check | Runtime evidence |
|---|---|---|---|---|
| AC-001 | FR-001, FR-002, FR-005 | Given an allowlisted Google account, when the user signs in and opens `/dashboard`, then exactly one `users` row exists with `credits = 3`, and a second visit creates no new row. | integration test (test DB) | screenshot of dashboard + DB row dump (sanitized) |
...
| AC-009 | NFR-001 | Given the route inventory, when the inventory test runs, then every `app/api/**/route.ts` either calls `requireUser()` or is in the documented allow-list (`webhooks/clerk`, `health`). | structural test in CI | CI log |
...
| AC-020 | FR-040 | Given a user at the daily run cap, when they send a message, then the runtime returns 429, no model call is made, and the chat shows the reset time. | runtime test with fake clock | probe + screenshot |
...
| AC-029 | FR-060 | Deferred with FR-060 (D-003). | — | — |
| AC-030 | FR-070, NFR-007 | Given the Railway service, when it is redeployed, then `GET /api/health` returns 200 within the health-check window and a board created before the redeploy opens with its scene intact. | e2e against Railway | health log + screenshot |
| AC-031 | FR-005 | Given an authenticated account not on the allowlist, when it opens `/dashboard` or calls `GET /api/projects`, then it sees the private-instance page / receives 403, no `users` row is created, and an `auth.denied` event is recorded. | integration + e2e | screenshot + probe + log excerpt |

## 13. Delivery slices

| Slice | Scope | Requirement links | Depends on | Reviewable outcome |
|---|---|---|---|---|
| S-1 | Bootstrap + governance: Next.js scaffold (in a temp dir, then moved in, because the repo is non-empty), Tailwind/shadcn, TS strict, ESLint incl. layer-import rules, vitest/Playwright skeleton, CI, `.coderabbit.yaml`, `GET /api/health`, real commands in `AGENTS.md`, `main` as protected default | NFR-002, NFR-005, AC-026, FR-070 (health) | D-001 | green CI on a PR; `AGENTS.md` commands verified |
| S-2 | Auth + DB + user sync: Clerk, allowlist in `requireUser()`, schema + constraints, scoped repos, audit helper, webhook | FR-001–FR-005, NFR-001, AC-001, AC-002, AC-009, AC-022, AC-031 | S-1 | allowlisted sign-in creates a user; others refused; inventory test passes |
...
| S-7 | Metering + Railway deploy: runtime credit/rate enforcement, `ai_usage`, Railway service + Neon, deploy on merge to `main`, observability, bundle secret grep; previews to object storage only if row size becomes a problem | FR-040, FR-070, NFR-003, NFR-005–NFR-009, AC-020, AC-026, AC-030 | S-6 | Railway URL; limits enforced; redeploy test passes |
...
S-6's pure functions (spec → layout → skeleton) may start in parallel with S-4. Railway service creation, variables, and domains are external actions performed in S-7 only with explicit authorization at that time.

## 14. Rollout, rollback, and observability

- **Rollout:** each slice ships behind the human merge gate; a single Railway service deploys from `main` on merge; AI tools can be disabled per environment with a flag. No separate staging in the personal-tool phase.
- **Rollback:** revert the merge commit (Railway redeploys); Drizzle migrations are additive through S-6 (no destructive migration without a documented down path).
- **Analytics/telemetry:** events `board.created`, `ai.run.completed` (tool names, element counts), `export.png`; no prompt text.
- **Operational monitoring:** error tracking, runtime `onError` logging, Railway health check and logs, provider spend dashboard with alert at 80 % of the monthly cap.
...
| In-memory chat threads reset on Railway redeploy | high / low | accepted for the personal-tool phase (A-010); revisit with a persistent runner if the product opens up | S-7 |
...
| Base64 previews bloat Postgres rows | low (one user) / low | size cap; object storage only if needed | S-7 |
...
| Budgets enforced only client-side by mistake | low / high | runtime hook + negative tests (AC-020) | S-7 |
| Public Railway URL reachable by anyone | high / medium | allowlist (FR-005) + Clerk sign-up restriction; negative test AC-031 | S-2 |
...
- `A-004` — Auth/DB stack is Clerk + Neon + Drizzle as in the transcript (Railway Postgres was not chosen; switching later is a connection-string change plus migration).
- `A-005` — Previews are stored as base64 JPEG (≤ 400 px, quality 0.5); object storage only if needed.
- `A-006` — Budgets: 3 board credits and a daily AI-run cap of 20, both configurable via environment variables since the owner is the only user.
...
- `A-009` — Personal-tool phase admits only allowlisted emails (`ALLOWED_EMAILS`) plus Clerk's sign-up restriction. Change by removing the allowlist check when opening the product.
- `A-010` — Chat threads live in the Railway process memory (`InMemoryAgentRunner`) and reset on redeploy. Change by adopting a persistent runner.

### Decisions

- `D-001` — **Open (owner action):** create `main` from the current commit, set it as GitHub default, protect it (require PR + CI + CodeRabbit). Blocks opening any slice PR.
- `D-002` — **Resolved 2026-09-16:** hosting is Railway (long-lived Node service).
- `D-003` — **Resolved 2026-09-16:** no billing and no Stripe; the product is used first as a personal tool. Credits remain as a cost control (A-006).
- `D-004` — Default stands (A-002, Anthropic Claude Opus 5). Reversible; say so to switch provider.
- `D-005` — Retention after account deletion (proposed: 30 days, then hard delete) and full export. Needed before S-2's webhook; low impact with one user.
- `D-006` — **Resolved by direction:** teams/sharing/realtime stay out of scope (personal tool).
...
- **Revision 2 (owner decisions):** hosting fixed to Railway (FR-070, AC-030, S-7 re-scoped); billing removed (FR-060/AC-029 deferred, non-goal); personal-tool phase adds the allowlist (FR-005, AC-031, A-009); in-memory chat threads accepted (A-010); D-002, D-003, D-006 resolved; budgets made configurable (A-006).
- **Clarified (revision 1):** every transcript feature is a numbered requirement with an observable criterion; "credits" covers AI runs, not only board creation; autosave interval 10 s → 2 s + tab-hide.
- **Added (revision 1):** server-side user sync, ownership checks on every route, idempotent create, payload caps, rate/spend budgets, audit events, HITL for destructive AI actions, compact diagram spec + layout pipeline, eval set, entry-point inventory test, governance (default branch), evidence plan.
- **Excluded:** Smart doc tab behavior, sharing/collaboration, teams, billing, FluidVoice code changes.
- **Unresolved:** D-001 (owner action), D-005.
